...
kbmf1mkl1mkl_semisupervised_regression_variational_train <- function(Kx, Kz, Y, parameters) {
  ...
  for (iter in 1:parameters$iteration) {
    ...
    for (i in 1:Nx) {
      ...
      Hx$sigma[,,i] <- chol2inv(chol(diag(1 / sigma_h^2, R, R) + (tcrossprod(Hz$mu[,indices, drop = FALSE], Hz$mu[,indices, drop = FALSE]) + apply(Hz$sigma[,,indices, drop = FALSE], 1:2, sum)) / sigma_y^2))
      Hx$mu[,i] <- tcrossprod(Hz$mu[,indices, drop = FALSE], Y[i, indices, drop = FALSE]) / sigma_y^2
      for (m in 1:Px) {
    ...
    for (j in 1:Nz) {
      ...
      Hz$sigma[,,j] <- chol2inv(chol(diag(1 / sigma_h^2, R, R) + (tcrossprod(Hx$mu[,indices, drop = FALSE], Hx$mu[,indices, drop = FALSE]) + apply(Hx$sigma[,,indices, drop = FALSE], 1:2, sum)) / sigma_y^2))

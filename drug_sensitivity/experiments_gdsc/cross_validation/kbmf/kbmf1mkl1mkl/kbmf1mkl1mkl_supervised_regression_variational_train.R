...
kbmf1mkl1mkl_supervised_regression_variational_train <- function(Kx, Kz, Y, parameters) {
  ...
  for (iter in 1:parameters$iteration) {
    ...
    # update Hx
    Hx$sigma <- chol2inv(chol(diag(1 / sigma_h^2, R, R) + (tcrossprod(Hz$mu, Hz$mu) + Nz * Hz$sigma) / sigma_y^2))
    ...
    # update Hz
    Hz$sigma <- chol2inv(chol(diag(1 / sigma_h^2, R, R) + (tcrossprod(Hx$mu, Hx$mu) + Nx * Hx$sigma) / sigma_y^2))

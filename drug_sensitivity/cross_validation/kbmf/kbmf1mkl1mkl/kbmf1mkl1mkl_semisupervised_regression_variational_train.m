...
function state = kbmf1mkl1mkl_semisupervised_regression_variational_train(Kx, Kz, Y, parameters)
    ...
    for iter = 1:parameters.iteration
        ...
        for i = 1:Nx
            indices = ~isnan(Y(i, :));
            Hx.sigma(:, :, i) = (eye(R, R) / sigma_h^2 + (Hz.mu(:, indices) * Hz.mu(:, indices)' + sum(Hz.sigma(:, :, indices), 3)) / sigma_y^2) \ eye(R, R);
            Hx.mu(:, i) = Hz.mu(:, indices) * Y(i, indices)' / sigma_y^2;
            for m = 1:Px
        ...
        for j = 1:Nz
            indices = ~isnan(Y(:, j));
            Hz.sigma(:, :, j) = (eye(R, R) / sigma_h^2 + (Hx.mu(:, indices) * Hx.mu(:, indices)' + sum(Hx.sigma(:, :, indices), 3)) / sigma_y^2) \ eye(R, R);
            Hz.mu(:, j) = Hx.mu(:, indices) * Y(indices, j) / sigma_y^2;
            for n = 1:Pz
